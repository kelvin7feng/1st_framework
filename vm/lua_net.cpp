...
//发送到目标服
static int SendToServer(lua_State* lua_state)
{
    int nRetCode = 0;
    ...
    unsigned short uTargetServerId = lua_tonumber(lua_state, nParam - 5);
    ...
    void* pNetPackage = CreateNetBuffer(uEventType, uErrorCode, uHandlerId, uTargetServerId, uSequenceId, szNetBody.c_str(), (unsigned int)szNetBody.length(), &uNetPacketSize);
    
    if(uTargetServerId == GATEWAY){
        GameLogicServer::GetInstance()->SendDataToGateway((char*)pNetPackage, uNetPacketSize);
    }
    else if(uTargetServerId == ROOM || uTargetServerId == LOGIC || uTargetServerId == LOGIN || uTargetServerId == CENTER)
    {
